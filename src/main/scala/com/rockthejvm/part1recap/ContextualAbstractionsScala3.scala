...
object ContextualAbstractionsScala3 {
  ...
  // given/using combo -- equivalent to implicit arguments and values
  def increment(x: Int)(using amount: Int): Int = x + amount
  given defaultAmount: Int = 10
  val twelve = increment(2)//(10) automatically passed by the compiler
  ...
  def combineAll[A](xs: List[A])(using combiner: Combiner[A]): A =
    xs.foldLeft(combiner.empty)(combiner.combine)
